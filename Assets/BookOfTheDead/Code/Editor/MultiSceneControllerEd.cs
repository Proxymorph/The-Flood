...
{
    [MenuItem("Tools/Load The FLOOD", priority = 0)]
    ...
    {
        EditorSceneManager.OpenScene("Assets/BookOfTheDead/Scenes/AssetLibrary.unity", OpenSceneMode.Single);
    }

    static public Scene LoadMasterScene(bool force = false)
    {
        var masterScene = EditorSceneManager.GetSceneByPath("Assets/BookOfTheDead/Scenes/Master.unity");

        if (force || !masterScene.IsValid())
            masterScene = EditorSceneManager.OpenScene("Assets/BookOfTheDead/Scenes/Master.unity", OpenSceneMode.Single);
